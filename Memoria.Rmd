...
## COMENTO ESTE CODIGO PORQUE DA FALLOS Y QUIERO EL VER HTML COMPILADO
```{r}
# credit.Datos.Todo <- rbind(credit.Datos.Train, credit.Datos.Test)
# 
...
# ggplot(data = credit.normalizadasCS) + 
#   geom_density(aes(x = Edad, fill = "Edad"), alpha = 0.5) +
#   geom_density(aes(x = Deuda, fill = "Deuda"), alpha = 0.5) +
#   geom_density(aes(x = AnyosContratado, fill = "Años Contratado"), alpha = 0.5) +
#   geom_density(aes(x = Solvencia, fill = "Solvencia"), alpha = 0.5) +
#   geom_density(aes(x = V14, fill = "V14"), alpha = 0.5) +
#   geom_density(aes(x = V15, fill = "V15"), alpha = 0.5) +
#   labs(
#     title = "Distribución de Variables Continuas (Density Plot)",
#     x = "Valor",
#     y = "Densidad"
...
library(caret)      # Para entrenar y validar el modelo
library(randomForest) # Para Random Forest (usado internamente por caret)
...
# 2. Verificar las estructuras de las bases de datos
# Asegúrate de que la variable respuesta esté presente y sea un factor (para clasificación)
str(credit.Datos.Train.SinNA)
str(credit.Datos.Test)

# 3. Definir variables predictoras y la variable objetivo
# Cambia "VariableObjetivo" por el nombre real de tu variable de salida
var_objetivo <- "V16"
vars_predictoras <- setdiff(names(credit.Datos.Train.SinNA), var_objetivo)

# 4. Asegurarse de que la variable objetivo sea un factor
credit.Datos.Train.SinNA[[var_objetivo]] <- as.factor(credit.Datos.Train.SinNA[[var_objetivo]])
#credit.Datos.Test[[var_objetivo]] <- as.factor(credit.Datos.Test[[var_objetivo]])

# 5. Configuración del control para validación cruzada
control_rf <- trainControl(
  method = "cv",               # Validación cruzada
  number = 5,                  # Número de folds
  verboseIter = TRUE,          # Mostrar progreso
  classProbs = TRUE,           # Calcular probabilidades de clase (si aplica)
  summaryFunction = twoClassSummary # Métricas adicionales como ROC (para clasificación binaria)
)

# 6. Definir el grid de hiperparámetros a probar
# Ajusta `mtry` al número de predictores en tus datos
tune_grid_rf <- expand.grid(
  mtry = seq(2, length(vars_predictoras), by = 2) # Rango de mtry a explorar
)

# 7. Entrenar el modelo Random Forest
set.seed(123) # Para reproducibilidad
modelo_rf <- train(
  x = credit.Datos.Train.SinNA[vars_predictoras], # Variables predictoras
  y = credit.Datos.Train.SinNA[[var_objetivo]],   # Variable objetivo
  method = "rf",                            # Random Forest
  trControl = control_rf,                   # Configuración de validación
  tuneGrid = tune_grid_rf,                  # Grid de hiperparámetros
  ntree = 500                               # Número de árboles
)

# 8. Resumen del modelo entrenado
print(modelo_rf)
plot(modelo_rf)
...
predicciones <- predict(modelo_rf, newdata = credit.Datos.Test[vars_predictoras])
...
```{r}
# 1. Cargar librerías necesarias
library(caret) # Para entrenar el modelo
library(nnet)  # Paquete para redes neuronales

# 2. Verificar las estructuras de las bases de datos
# Define el nombre de la variable objetivo y las variables predictoras
var_objetivo <- "V16" # Cambia esto por tu variable de salida
vars_predictoras <- setdiff(names(credit.Datos.Train), var_objetivo)


# 3. Configuración del control para validación cruzada
control_nnet <- trainControl(
  method = "cv",               # Validación cruzada
  number = 5,                  # Número de folds
  verboseIter = TRUE,          # Mostrar progreso
...
# 4. Definir el grid de hiperparámetros
tune_grid_nnet <- expand.grid(
  size = c(1, 3, 5, 7),  # Número de nodos en la capa oculta
  decay = c(0, 0.01, 0.1) # Parámetro de regularización (previene sobreajuste)
)

# 5. Entrenar el modelo con nnet
set.seed(123) # Para reproducibilidad
modelo_nnet <- train(
  x = credit.Datos.Train[vars_predictoras], # Variables predictoras
  y = credit.Datos.Train[[var_objetivo]],   # Variable objetivo
  method = "nnet",                          # Especificamos nnet
  trControl = control_nnet,                 # Configuración de validación
  tuneGrid = tune_grid_nnet,                # Grid de hiperparámetros
  trace = FALSE,                            # Evita imprimir mensajes internos de nnet
  maxit = 200                               # Número máximo de iteraciones
)

# 6. Resumen del modelo entrenado
print(modelo_nnet)
plot(modelo_nnet)
