...
print(rf.modelo.3cv10.twoSummary.roc.100$results$ROC[4])
```

Vamos a ver con el siguiente codigo cual es el maximo ROC de cada uno de los modelos y para que hiperparametro, recordemos que cuanto mas cercanos sea el ROC a uno mejor predicciones tendrá el modelo.
...
Vemos que probablemente el modelo se comporta mejor con valores pequeños de mtry. Ahora vemos una comparativa segun el nummero de arboles.

```{r}
# Crear un data.frame con los resultados
resultados_df <- data.frame(
  mtry = rep(c(1, 2, 6, 10), times = 3), # Repetimos 4 valores de mtry, una vez por modelo
  ntree = rep(c(100, 500, 1000), each = 4), # 3 modelos, cada uno con 4 mtry
  ROC = c(
    rf.modelo.3cv10.twoSummary.roc.100$results$ROC[1:4], 
    rf.modelo.3cv10.twoSummary.roc.500$results$ROC[1:4], 
    rf.modelo.3cv10.twoSummary.roc.1000$results$ROC[1:4]
...
# Crear un data.frame con los resultados
resultados_df <- data.frame(
  mtry = rep(c(1, 2, 6, 8, 10), times = 3), # Repetimos 4 valores de mtry, una vez por modelo
  ntree = rep(c(100, 500, 1000), each = 4), # 3 modelos, cada uno con 4 mtry
  ROC = c(
    rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA$results$ROC[1:4], 
    rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA$results$ROC[1:4], 
    rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA$results$ROC[1:4]
...
ggplot(resultados_df, aes(x = as.factor(mtry), y = ROC, fill = as.factor(ntree))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "ROC según Número de mtry y Número de Árboles (Eliminados NA)",
    x = "Número de mtry",
    y = "ROC",
    fill = "Número de Árboles (ntree)"
  ) +
