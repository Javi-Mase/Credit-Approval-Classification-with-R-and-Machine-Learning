...
### Transformacion de distribuciones asimétricas
...
Este paso también debe realizarse en todos los prepocesados que queramos probar.
...
El porcentaje de acierto del modelo sobre el conjunto de datos entrenamiento del modelo es el siguiente:
...
Sin embargo, este resultado no es válido para evaluar el desempeño real del modelo, ya que mide el rendimiento sobre los datos con los que el modelo ha entrenado. Esto puede generar un resultado más optimista que el error real, ya que el modelo ha "visto" estos datos durante el entrenamiento y puede estar ajustado a ellos.

Es por esto que vamos a medir el rendimiento sobre el conjunto test.
...
### **¿Con qué configuración hemos hecho los entrenamientos?**
...
Tras las ejecuciones, consideramos que para los datos del modelo SVM realizaremos las siguientes acciones:
...
No será necesario normalizar los datos, ya que PCA incluye un paso de estandarización de las variables de forma implícita.
...
Como podemos observar la eliminación de la variable PC5, que no tenía impotancia el modelo final entrenado, supone un aumento en el porcentaje de acierto.

### Conclusión

Para la configuración de nuestro modelo SVM, el mejor resultado lo obtienen los datos tras haber hecho Dummys, eliminado las variables con poca varianza y habiendo aplicado PCA. Tras todo esto eliminamos PC5 y de esta forma obtenemos un porcentaje de acierto de 87.59%.
...
Como ya sabemos ese proporcionará un resultado más optimista que el real. El resultado final del modelo sería:
...
En nuestro caso k=13 proporcionara un equilibrio entre la sensibilidad al ruido y la generalidad.

### **¿Con qué configuración hemos hecho los entrenamientos?**

Para el modelo knn es necesario haber hecho dummys, una vez hecho esto probamos varios preprocesados.
...
En este caso, el mejor resultado se obtiene después de haber normalizado los datos mediante el método Range y haber transformado las variables categóricas en dummys.
...
Para el modelo de Knn, el mejor resultado lo obtenemos tras una normalización y la transformación de las variables categóricas. El modelo obtendría un porcentaje de acierto de 87.59 %.
...
Lo que no sabe gestioniar RF son los valores NA por lo que ahi si que haremos un preprocesado de los datos. Trataremos este tema de dos formas distintas, imputando los valores NA y eliminandolos y asi veremos cual de las dos tecnicas ofrece mejores resultados.
...
Vemos que efectivamente no hay datos NA por lo que creamos nuestra variable objtivo y nuestras variables predictoras.
...
Otra manera de preprocesar los datos para el random forest es eliminando los valores NA, para ello usamos lo siguiente.
...
```{r}
any(is.na(credit.Datos.Train.EliminadoNA))
```

Como hemos comprobado no hay valores NA por lo que creamos nuestra variable objetivo y nuestras variables predictoras.
...
Vamos a ver con el siguiente codigo cual es el maximo ROC de cada uno de los modelos y para que hiperparametro, recordemos que cuanto mas cercanos sea el ROC a uno mejores predicciones tendrá el modelo.
...
ggplot(resultados_df, aes(x = as.factor(mtry), y = ROC, fill = as.factor(ntree))) +
  ...
  coord_cartesian(ylim = c(0.9, 1))  # Ajustar el rango del eje Y
...
Repetimos el mismo codigo pero esta vez para la base con los datos NA eliminados.

```{r}

# Entrenamiento para 100 arboles
set.seed(123)
rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA <- train(
...
# Entrenamiento para 500 arboles
set.seed(123)
rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA <- train(
  x = credit.Datos.Train.EliminadoNA[varsPredictoras],
  y = credit.Datos.Train.EliminadoNA[[varObjetivo]],
  method = "rf", 
  metric = "ROC",
...
# Entrenamiento para 1000 arboles
set.seed(123)
rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA <- train(
  x = credit.Datos.Train.EliminadoNA[varsPredictoras],
  y = credit.Datos.Train.EliminadoNA[[varObjetivo]],
  method = "rf", 
  metric = "ROC",
...
Como en la subseccion anterior vemos qu evalor de mytry ha dado el valores mas alto.

```{r}
maxIndexRf100Eliminado <- which.max(rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA$results$ROC)

# Obtener los valores correspondientes
maxRocRf100Eliminado <- rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA$results$ROC[maxIndexRf100Eliminado]

mtry100Eliminado <- rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA$results$mtry[maxIndexRf100Eliminado]

# Imprimir los resultados
cat("El valor más alto de ROC para 100 arboles es:", maxRocRf100Eliminado, "\n")
cat("Corresponde a mtry:", mtry100Eliminado, "\n")


maxIndexRf500Eliminado <- which.max(rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA$results$ROC)

# Obtener los valores correspondientes
maxRocRf500Eliminado <- rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA$results$ROC[maxIndexRf500Eliminado]

mtry500Eliminado <- rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA$results$mtry[maxIndexRf500Eliminado]

# Imprimir los resultados
cat("El valor más alto de ROC para 500 arboles es:", maxRocRf500Eliminado, "\n")
cat("Corresponde a mtry:", mtry500Eliminado, "\n")


maxIndexRf1000Eliminado <- which.max(rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA$results$ROC)

# Obtener los valores correspondientes
maxRocRf1000Eliminado <- rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA$results$ROC[maxIndexRf1000Eliminado]

mtry1000Eliminado <- rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA$results$mtry[maxIndexRf1000Eliminado]

# Imprimir los resultados
cat("El valor más alto de ROC para 1000 arboles es:", maxRocRf1000Eliminado, "\n")
cat("Corresponde a mtry:", mtry1000Eliminado, "\n")


```

Podemos observar que en cuanto al ROC los valores no difieren a penas de los entrenamientos con los datos NA imputados, veremos ahora como se comportan en los test. Lo que si podemos decir es que siguen una forma parecida a los entrenamientos anteriores, es decir, el valor mtry 4 es el que mejor resultados nos da y que a partir de 500 arboles no mejora mucho la precision del modelo.
...
resultados_df <- data.frame(
  ...
  ntree = rep(c(100, 500, 1000), each = 5), # 3 modelos, cada uno con 5 valores de mtry
  ROC = c(
    rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA$results$ROC[1:5], 
    rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA$results$ROC[1:5], 
    rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA$results$ROC[1:5]
  )   
)
...
Ahora antes de testear los modelos, vamos a hacer un nuevo entrenamiento con los hiperparametros que mejor resultado han dado:
...
# ENtrenamiento con valores NA eliminados
prediccion.rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA <- predict(rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA, newdata = credit.Datos.Test.imp[varsPredictoras])

prediccion.rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA <- predict(rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA, newdata = credit.Datos.Test.imp[varsPredictoras])

prediccion.rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA <- predict(rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA, newdata = credit.Datos.Test.imp[varsPredictoras])
...
matrizConfusion.rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA <- confusionMatrix(
  data = prediccion.rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA,
  reference = credit.Datos.Test[[varObjetivo]],
  positive = "aprobada" # Cambia esto por la clase objetivo positiva
)

matrizConfusion.rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA <- confusionMatrix(
  data = prediccion.rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA,
  reference = credit.Datos.Test[[varObjetivo]],
  positive = "aprobada" # Cambia esto por la clase objetivo positiva
)

matrizConfusion.rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA <- confusionMatrix(
  data = prediccion.rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA,
  reference = credit.Datos.Test[[varObjetivo]],
  positive = "aprobada" # Cambia esto por la clase objetivo positiva
)
...
accuracy_eliminados <- c(
  matrizConfusion.rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA$overall["Accuracy"] * 100,
  matrizConfusion.rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA$overall["Accuracy"] * 100,
  matrizConfusion.rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA$overall["Accuracy"] * 100
)

# Crear un data frame con los resultados
resultados <- data.frame(
  Numero_Arboles = rep(c(100, 500, 1000), times = 2),
  Tipo_Datos = rep(c("Imputados", "Eliminados"), each = 3),
  Accuracy = c(accuracy_imputados, accuracy_eliminados)
)


# Crear el histograma
ggplot(resultados, aes(x = as.factor(Numero_Arboles), y = Accuracy, fill = Tipo_Datos)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Accuracy por Número de Árboles y Tratamiento de NA",
    x = "Número de Árboles",
    y = "Accuracy (%)",
    fill = "Tipo de Datos"
  ) +
  ...
  scale_fill_manual(values = c("Imputados" = "blue", "Eliminados" = "orange"))
...
varImp(rf.modelo.3cv10.twoSummary.roc.1000)

varImp(rf.modelo.3cv10.twoSummary.roc.100.eliminadoNA)

varImp(rf.modelo.3cv10.twoSummary.roc.500.eliminadoNA)

varImp(rf.modelo.3cv10.twoSummary.roc.1000.eliminadoNA)
```

Vemos que la V7 tiene un impacto infimos en todos los modelos por lo que vamos a eliminarla. También se podria considerar la eliminación de la variable de empleados pero hay algunos modelos que si que han hecho un uso considerable para no eliminarla.

```{r}
predictors(rf.modelo.3cv10.twoSummary.roc.100$finalModel)
```
...
nnetGrid <- expand.grid(
  size = c(1, 3, 5, 7, 10),
  decay = c(0.01, 0.1, 0.5, 1, 2)
)
```

### Configaración de trainControl()

Usamos la misma configuaración que en random forest
...
Todos los preprocesados siguientes ya han sido hechos y estan en la base de datos credit.Datos.Train.Knn por lo que la reutilizamos para evitar repetición de codigo. Por lo que simplemente los describiremos.

Antes de nada creamos nuestra variable objetivo y nuestras variables predictoras.

```{r}
varObjetivo <- "V16"
varsPredictoras <- setdiff(names(credit.Datos.Train.Knn), varObjetivo)
```

#### Imputación de valores NA

Nnet no puede maneja valores NA por lo que usaremos la base de datos donde los valores NA estan imputados.
...
#### Eliminación de Predictores Irrelevantes o Redundantes
...
nnet.train.100 <- train(
  x = credit.Datos.Train.Knn[varsPredictoras],
  y = credit.Datos.Train.Knn[[varObjetivo]],
  method = "nnet",
...
```{r}
# print(nnet.train.100)
# print(nnet.train.300)
# print(nnet.train.500)
...
# Encontrar el índice del máximo valor de ROC
max_index500 <- which.max(nnet.train.500$results$ROC)

# Obtener los valores correspondientes
max_roc500 <- nnet.train.500$results$ROC[max_index500]
corresponding_size500 <- nnet.train.500$results$size[max_index500]
corresponding_decay500 <- nnet.train.500$results$decay[max_index500]

# Imprimir los resultados
cat("El valor más alto de ROC para 500 iteraciones es:", max_roc500, "\n")
cat("Corresponde a size:", corresponding_size500, "y decay:", corresponding_decay500, "\n")
...
prediccion.nnet.100 <- predict(nnet.train.100, newdata = credit.Datos.Test.Knn[varsPredictoras])

prediccion.nnet.300 <- predict(nnet.train.300, newdata = credit.Datos.Test.Knn[varsPredictoras])

prediccion.nnet.500 <- predict(nnet.train.500, newdata = credit.Datos.Test.Knn[varsPredictoras])
...
matrizConfusion.nnet.500 <- confusionMatrix(
  data = prediccion.nnet.500,
  reference = credit.Datos.Test[[varObjetivo]],
  positive = "aprobada" # Cambia esto por la clase objetivo positiva
)
...
accuracy_values <- c(
  ...
  matrizConfusion.nnet.500$overall["Accuracy"] * 100
)

# Crear un data frame con los resultados
resultados <- data.frame(
  Modelo = c("100 iteraciones", "300 iteraciones", "500 iteraciones"),
