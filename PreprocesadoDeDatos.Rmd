...
Como en la subseccion anterior vemos que valor de mytry ha dado el valores mas alto.
...
ggplot(resultados_df, aes(x = as.factor(mtry), y = ROC, fill = as.factor(ntree))) +
  ...
  theme_minimal() +
  theme(legend.position = "bottom") +
  coord_cartesian(ylim = c(0.9, 0.975))  # Ajustar el rango del eje Y
  # Ajustar el rango del eje Y
```
Como pasaba con los valores NA imputados, ocurre lo mismo con los valores NA elimiandos que el rendimiento no mejores sustancialmente a partir de 500 arboles, es mas en algunos casos empeora por lo que 500 arboles es la mejor opción.

Tambien se ve una ligera mejoria (casi descartable) respecto a los valores NA imptados, esto se puede deber a que un valor imputado esta dando mas informacion que un valor eliminado por lo que usaremos para futuros entrenamiento la base de datos con los NA imputados.
...
ggplot(resultados, aes(x = as.factor(Numero_Arboles), y = Accuracy, fill = Tipo_Datos)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Accuracy por Número de Árboles (NA eliminado)",
    x = "Número de Árboles",
    y = "Accuracy (%)",
    fill = "Tipo de Datos"
  ) +
...
pima.modelList.3cv10.tl7.rsAll.Acc<-list(
  KNN=credit.modelo.knn
 ,SVM=credit.modelo.final.svm
 ,RF=rf.modelo.3cv10.twoSummary.roc.100.final
 ,GBM=modelo_final_gbm
 ,NNET=nnet.train.100.final
 )

pima.resamps.3cv10.tl7.rsAll.Acc<-resamples(pima.modelList.3cv10.tl7.rsAll.Acc)
summary(pima.modelList.3cv10.tl7.rsAll.Acc)
